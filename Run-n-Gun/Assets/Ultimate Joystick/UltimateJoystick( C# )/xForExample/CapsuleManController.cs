...
/* CapsuleManController.cs ver. 1.2 */
using UnityEngine;
...
public class CapsuleManController : MonoBehaviour
{
	...
	void PlayerJumpCheck ()
	{
		...
		if( isGrounded == true )
		{
			if( joystickRight.tapCountOption != UltimateJoystick.TapCountOption.NoCount )
			{
				// Store a local int to reference our joystickRight.JoystickTapCount
				int tapCount = joystickRight.JoystickTapCount;
				if( joystickRight.tapCountOption == UltimateJoystick.TapCountOption.TouchRelease )
				{
	...
	IEnumerator DelayJump ()
	{
		...
		for( float t = 0.0f; t < 1.0f && continueCheck == true; t += Time.deltaTime * 2 )
		{
			int tapCount = joystickRight.JoystickTapCount;
			if( tapCount == 0 )
			{
		...
		joystickRight.JoystickTapCount = 0;
		delayJump = false;
	}
